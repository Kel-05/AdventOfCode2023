...
void array(char scanner[3][7], char *str_number, int row, int column, int start, int size) {
  ...
  for(int i = 0; start < size-1; start++) {
...
void reverse_array(char scanner[3][7], char *str_number, int row, int column, int size) {
  ...
  for(int y = 0; y < 3; y++) {
    for(int x = 2; x < 5; x++) {
      ...
      if(isdigit(scanner[y][x]) && isdigit(scanner[y][x-1])) {
        reverse_array(scanner, str_number, y, x, sizeof(str_number));
      }
      ...
      else if(isdigit(scanner[y][x]) && strlen(str_number) == 0) {  // if none of the previous conditions was true and there is still
        str_number[0] = scanner[y][x];                              // a digit in x, then just add it to the first position
      }
      
      // if 2 numbers are found, multiply them 
      if(scanner[1][3] == '*' && buffer != 0 && strlen(str_number) != 0) {
        sum = buffer * atoi(str_number);
      }
      
      if(strlen(str_number) != 0) { // if array is not empty
        buffer = atoi(str_number); // store number inside a buffer
